...
namespace NzbDrone.Core.Datastore
{
        ...
        where TModel : ModelBase, new()
    {
        ...
        {
            var results = await _database.QueryAsync<TModel>(builder, cancellationToken).ConfigureAwait(false);
        ...
        {
            var results = await _database.QueryDistinctAsync<TModel>(builder, cancellationToken).ConfigureAwait(false);
            return results.ToList();
        }
