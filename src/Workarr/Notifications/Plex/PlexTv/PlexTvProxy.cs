...
namespace Workarr.Notifications.Plex.PlexTv
{
    ...
    public class PlexTvProxy : IPlexTvProxy
    {
        ...
        private string ProcessRequest(HttpRequestBuilder requestBuilder)
        {
            ...
            {
                throw new WorkarrClientException(ex.Response.StatusCode, "Unable to connect to plex.tv");
            }
            catch (WebException)
            {
                throw new WorkarrClientException(HttpStatusCode.BadRequest, "Unable to connect to plex.tv");
            }
