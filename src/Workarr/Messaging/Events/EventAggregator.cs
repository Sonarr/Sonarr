...
namespace Workarr.Messaging.Events
{
    public class EventAggregator : IEventAggregator
    {
            ...
            where TEvent : class, IEvent
        {
            ...
            {
                _syncHandlers = Enumerable
                                              .OrderBy<IHandle<TEvent>, int>(serviceFactory.BuildAll<IHandle<TEvent>>(), GetEventHandleOrder<TEvent>)
                                              .ToArray();

                _globalHandlers = serviceFactory.BuildAll<IHandleAsync<IEvent>>()
                                              .ToArray();

                _asyncHandlers = Enumerable
                                               .ToArray<IHandleAsync<TEvent>>(serviceFactory.BuildAll<IHandleAsync<TEvent>>());
            }
        }
            ...
            where TEvent : class, IEvent
        {
            ...
            var eventName = GetEventName(((Object)@event).GetType());
