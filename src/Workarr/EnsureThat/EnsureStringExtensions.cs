...
namespace Workarr.EnsureThat
{
    ...
    public static class EnsureStringExtensions
    {
        ...
        public static Param<string> HasLengthBetween(this Param<string> param, int minLength, int maxLength)
        {
            ...
            if (length < minLength)
            {
                throw ExceptionFactory.CreateForParamValidation(param.Name, StringExtensions.Inject(ExceptionMessages.EnsureExtensions_IsNotInRange_ToShort, minLength, maxLength, length));
            }

            if (length > maxLength)
            {
                throw ExceptionFactory.CreateForParamValidation(param.Name, StringExtensions.Inject(ExceptionMessages.EnsureExtensions_IsNotInRange_ToLong, minLength, maxLength, length));
            }
        ...
        public static Param<string> Matches(this Param<string> param, Regex match)
        {
            ...
            {
                throw ExceptionFactory.CreateForParamValidation(param.Name, StringExtensions.Inject(param.Value, match));
            }
