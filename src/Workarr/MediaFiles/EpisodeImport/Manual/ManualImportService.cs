    ...
    public class ManualImportService : IExecute<ManualImportCommand>, IManualImportService
    {
            ...
            if (!seasonNumber.HasValue)
            {
                ...
                items.AddRange(unmappedFiles.Select(file =>
                    new ManualImportItem
                    {
                        ...
                        RelativePath = PathExtensions.GetRelativePath(series.Path, file),
        ...
        public ManualImportItem ReprocessItem(string path, string downloadId, int seriesId, int? seasonNumber, List<int> episodeIds, string releaseGroup, QualityModel quality, List<Language> languages, int indexerFlags, ReleaseType releaseType)
        {
            ...
            if (languageParse.Count <= 1 && languageParse.First() == Language.Unknown && series != null)
            {
                ...
                _logger.Debug((string)"Language couldn't be parsed from release, falling back to series original language: {0}", (string)series.OriginalLanguage.Name);
            }
            ...
            if (episodeIds.Any())
            {
                ...
                localEpisode.ExistingFile = PathExtensions.IsParentPath(series.Path, path);
            ...
            if (seasonNumber.HasValue)
            {
                ...
                var localEpisode = new LocalEpisode
                {
                    ...
                    ExistingFile = PathExtensions.IsParentPath(series.Path, path),
        ...
        private List<ManualImportItem> ProcessFolder(string rootFolder, string baseFolder, string downloadId, int? seriesId, bool filterExistingFiles)
        {
            ...
            else
            {
                ...
                {
                    _logger.Warn((Exception)e, "Unable to find series from title");
                }
            }
        ...
        {
            return !(PathExtensions.PathEquals(series.Path, folder) || PathExtensions.IsParentPath(series.Path, folder));
        }
        ...
        private ManualImportItem MapItem(ImportDecision decision, string rootFolder, string downloadId, string folderName)
        {
            ...
            item.Name = Path.GetFileNameWithoutExtension((string)decision.LocalEpisode.Path);
            item.DownloadId = downloadId;

            if (decision.LocalEpisode.Episodes.Any() && Enumerable.Distinct<int>(decision.LocalEpisode.Episodes.Select(c => c.SeasonNumber)).Count() == 1)
            {
                var seasons = Enumerable.Distinct<int>(decision.LocalEpisode.Episodes.Select(c => c.SeasonNumber)).ToList();

                if (seasons.Empty())
                {
                    _logger.Warn((string)"Expected one season, but found none for: {0}", (string)decision.LocalEpisode.Path);
                }
                else if (seasons.Count > 1)
                {
                    _logger.Warn<int, string, string>("Expected one season, but found {0} ({1}) for: {2}", seasons.Count, string.Join(", ", seasons), decision.LocalEpisode.Path);
                }
                else
                {
        ...
        public void Execute(ManualImportCommand message)
        {
            ...
            for (var i = 0; i < message.Files.Count; i++)
            {
                ...
                var existingFile = PathExtensions.IsParentPath(series.Path, file.Path);
            ...
            if (untrackedImports.Any())
            {
                foreach (var groupedUntrackedImport in untrackedImports.GroupBy(i => new { i.EpisodeFile.SeriesId, i.EpisodeFile.SeasonNumber }))
                {
                    ...
                    var sourcePath = localEpisodes.Select(l => l.Path).ToList<string>().GetLongestCommonPath();
            ...
            foreach (var groupedTrackedDownload in importedTrackedDownload.GroupBy<ManuallyImportedFile, string>(i => i.TrackedDownload.DownloadItem.DownloadId).ToList())
            {
