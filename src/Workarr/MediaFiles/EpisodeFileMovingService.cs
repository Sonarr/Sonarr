...
namespace Workarr.MediaFiles
{
    ...
    public class EpisodeFileMovingService : IMoveEpisodeFiles
    {
        ...
        private EpisodeFile MoveEpisodeFile(EpisodeFile episodeFile, Series series, List<Episode> episodes)
        {
            ...
            EnsureEpisodeFolder(episodeFile, series, episodes.Select(v => v.SeasonNumber).First<int>(), filePath);

            _logger.Debug<EpisodeFile, string>("Renaming episode file: {0} to {1}", episodeFile, filePath);
        ...
        {
            var filePath = _buildFileNames.BuildFilePath(localEpisode.Episodes, localEpisode.Series, episodeFile, Path.GetExtension((string)localEpisode.Path), null, localEpisode.CustomFormats);

            EnsureEpisodeFolder(episodeFile, localEpisode, filePath);

            _logger.Debug<string, string>("Moving episode file: {0} to {1}", episodeFile.Path, filePath);
        ...
        public EpisodeFile CopyEpisodeFile(EpisodeFile episodeFile, LocalEpisode localEpisode)
        {
            ...
            {
                _logger.Debug<string, string>("Attempting to hardlink episode file: {0} to {1}", episodeFile.Path, filePath);
            ...
            _logger.Debug<string, string>("Copying episode file: {0} to {1}", episodeFile.Path, filePath);
        ...
        private EpisodeFile TransferFile(EpisodeFile episodeFile, Series series, List<Episode> episodes, string destinationFilePath, TransferMode mode, LocalEpisode localEpisode = null)
        {
            ...
            episodeFile.RelativePath = PathExtensions.GetRelativePath(series.Path, destinationFilePath);
