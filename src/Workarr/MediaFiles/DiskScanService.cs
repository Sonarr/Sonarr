...
namespace Workarr.MediaFiles
{
        ...
        IExecute<RescanSeriesCommand>
    {
        ...
        public void Scan(Series series)
        {
            ...
            if (!seriesFolderExists)
            {
                if (_configService.CreateEmptySeriesFolders)
                {
                    ...
                    {
                        _logger.Debug((string)"Not creating missing series folder: {0} because delete empty series folders is enabled", (string)series.Path);
                    }
                    else
                    {
                        _logger.Debug((string)"Creating missing series folder: {0}", (string)series.Path);
                ...
                {
                    _logger.Debug((string)"Series folder doesn't exist: {0}", (string)series.Path);
                }
        ...
        {
            _logger.Info((string)"Completed scanning disk for {0}", (string)series.Title);
