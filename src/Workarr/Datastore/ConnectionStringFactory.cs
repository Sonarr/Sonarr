...
namespace Workarr.Datastore
{
    ...
    public class ConnectionStringFactory : IConnectionStringFactory
    {
        ...
        public ConnectionStringFactory(IAppFolderInfo appFolderInfo, IConfigFileProvider configFileProvider)
        {
            ...
            MainDbConnection = StringExtensions.IsNotNullOrWhiteSpace(_configFileProvider.PostgresHost) ? GetPostgresConnectionString(_configFileProvider.PostgresMainDb) :
                GetConnectionString(appFolderInfo.GetDatabase());

            LogDbConnection = StringExtensions.IsNotNullOrWhiteSpace(_configFileProvider.PostgresHost) ? GetPostgresConnectionString(_configFileProvider.PostgresLogDb) :
