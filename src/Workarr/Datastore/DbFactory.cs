...
namespace Workarr.Datastore
{
    ...
    {
        private static readonly Logger Logger = WorkarrLogger.GetLogger(typeof(DbFactory));
                ...
                if (e.InnerException is SocketException)
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new WorkarrStartupException(ex, "Error creating main database");
                        }
                    }
                }
                else
                {
                    throw new WorkarrStartupException(e, "Error creating main database");
                }
            }
            catch (Exception e)
            {
        ...
        private void CreateLog(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            {
                throw new WorkarrStartupException(e, "Error creating log database");
            }
        }
    }
}
