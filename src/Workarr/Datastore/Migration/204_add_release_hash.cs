        ...
        private void UpdateEpisodeFiles(IDbConnection conn, IDbTransaction tran)
        {
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (parsedEpisodeInfo == null || StringExtensions.IsNullOrWhiteSpace(parsedEpisodeInfo.ReleaseHash))
                    {
                        parsedEpisodeInfo = Parser.Parser.ParseTitle(Path.GetFileNameWithoutExtension(relativePath));
                    }

                    if (parsedEpisodeInfo != null && StringExtensions.IsNotNullOrWhiteSpace(parsedEpisodeInfo.ReleaseHash))
                    {
                        updates.Add(new
                        {
                            Id = id,
