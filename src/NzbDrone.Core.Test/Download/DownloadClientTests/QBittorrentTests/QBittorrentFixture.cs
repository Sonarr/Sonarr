...
namespace NzbDrone.Core.Test.Download.DownloadClientTests.QBittorrentTests
{
    ...
    public class QBittorrentFixture : DownloadClientFixtureBase<QBittorrent>
    {
        ...
        [Test]
        public void Download_should_blocklist_and_throw_when_torrent_contains_dangerous_file()
        {
            ...
            indexer.Definition = new IndexerDefinition
            {
                ...
                {
                    FailDownloads = new List<int> { (int)FailDownloads.Executables, (int)FailDownloads.PotentiallyDangerous }
                }
            };
            ...
            Mocker.GetMock<IQBittorrentProxy>()
                  .Verify(s => s.AddTorrentFromFile(It.IsAny<string>(), It.IsAny<byte[]>(), It.IsAny<TorrentSeedConfiguration>(), It.IsAny<QBittorrentSettings>()), Times.Never());
        }

        [Test]
        public async Task Download_should_not_check_torrent_files_when_fail_downloads_is_not_enabled()
        {
            GivenSuccessfulDownload();
        ...
        [Test]
        public async Task Download_should_not_blocklist_when_torrent_contains_only_safe_files()
        {
            GivenSuccessfulDownload();
            ...
            Mocker.GetMock<ITorrentFileInfoReader>()
                  .Setup(s => s.GetFileNamesFromTorrentFile(It.IsAny<byte[]>()))
                  .Returns(new List<string> { "Droned.S01E01.Pilot.1080p.WEB-DL-DRONE.mkv", "Sample/sample.mkv" });
